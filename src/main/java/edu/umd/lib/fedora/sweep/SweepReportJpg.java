      ...
      while ((strPid = buffRead.readLine()) != null) {
        ...
        if (bProcessThisOne) {
          ...
          if (oUMDM != null) {
            ...
            if (hRecord.containsKey("umamPid")) {
              // We use the lParts from up above
              strTemp = "";
              String[] aFile;
              for (String sPart : lParts) {
                
                // Reinitialize the UMAM parts
                if( hRecord.containsKey("file")) {
                  hRecord.put("file", "");
                }
                if( hRecord.containsKey("umamPid")) {
                  hRecord.put("umamPid", "");
                ...
                if( aFile.length > 1 ) {
                  if( hRecord.containsKey("file")) {
                    hRecord.put("file", aFile[1]);
                  }
                }
                
                if( aFile.length > 0 ) {
                  if( hRecord.containsKey("umamPid")) {
                    hRecord.put("umamPid", aFile[0]);
                  }
                }
...
//    }
  ...
  public static void main ( String[] args ) {
    ...
    SweepReportJpg thisSweep = new SweepReportJpg();
    thisSweep.Process();
