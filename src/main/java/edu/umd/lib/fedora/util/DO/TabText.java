...
public class TabText implements Iterable<HashMap<String, String>>,
    ...
    if (hasNext()) {
      ...
      try {
        ...
        if ((line = in.readLine()) != null) {
          ...
          // Some lines may be shorter than the header - deal with it.
          int iLineSize = lineRecord.size();
  ...
  private List<String> parse(String line) {
    ...
    /*
     * The Regex above: \\G(?:^|\t)(?:\"((?:[^\"]*+|\"\")*+)\"|([^\"\t]*)) has
     * only 2 groups: one for quoted and one for unquoted fields. Group 1:
     * Quoted Fields (with and without embedded double quotes) Group 2: Unquoted
     * fields (numeric and null fields). If the Regex is changed, the clauses
