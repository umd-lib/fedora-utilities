...
public class METSxml {
  ...
  // Constants to get the members of the getParts structure
  public static final int PART_PID = 0; // umd:########
  public static final int PART_LABEL = 1; // Part 1 of 23
  public static final int PART_TYPE = 2; // DISPLAY, MASTER, MASTER_RETOUCHED
  public static final int PART_POSITION = 3; // (order) 1, 2, 3, ...
  public static final int PART_KIND = 4; // image, video, or related

  private String strCreationDate = "";

  // collection[0] is the primary collection
  private final ArrayList<String> aTypes;
  private final ArrayList<String> collections;

  /*
   * members is the array of pids that are associated with this object. The
   * position of the pid in the array will be the basis for the fptr FILEID.
   * arrays are zero based and the FILEID is one based soooooooo index + 1 =
   * FILEID for the particular member.
   */
  private ArrayList<String> members;
  private final ArrayList<HashMap<String, String>> hasImages;
  private final ArrayList<HashMap<String, String>> hasVideos;
  private final ArrayList<String> hasRelations;

  /*
   * hasImages kinda looks like this: {["LABEL"->"Page 1" "DISPLAY"->"umd:3030"
   * "MASTER"->"umd:3031" "MASTER-RETOUCHED"->"umd:3032"], [{"LABEL"->"Page 2"
   * "DISPLAY"->"5" "MASTER"->"6" "MASTER-RETOUCHED"->"7"] Order will be the
   * key.
   */
  /*
   * hasRelations is an array of pids that indicate related objects The position
   * of the pid in the array will be the basis for the ORDER. arrays are zero
   * based and the ORDER is one based soooooooo index + 1 = ORDER for the
   * particular member.
   */
  ...
  public METSxml(String newType) {
    ...
    if (newType != null && !newType.equalsIgnoreCase("images")) {
   ...
   * @param label
   */
  public void addPart(int memberPos, String memberPid, String type, String label) {
    ...
    if (bIsStillGood) {
      ...
      System.out.println("Setting part " + memberPos);
      hThisOne.put("LABEL", label);
      hThisOne.put(type, memberPid);
      // hThisOne.put("fileName", fileName);

      // System.out.println("METS Position " + memberPos);
      // System.out.println("METS Label " + hThisOne.get("LABEL"));
      // System.out.println("METS pid " + hThisOne.get(type));
      // System.out.println("METS FileName " + hThisOne.get("fileName"));

      if (memberPos <= iSize) {
        hasParts.set((memberPos - 1), hThisOne);
      } else {
  ...
  public boolean isOK() {
    if (collections.size() > 0
        && (hasImages.size() > 0 || hasVideos.size() > 0)) {
      System.out.println("Everything checks out in the METS");
      return true;
    } else {

      // log.error("Bad METS: " + collections.size() + "-" + hasImages.size()
      // + "-" + hasVideos.size());
      return false;
    }
  }
  ...
  public Document getXML() {
    ...
    if (isOK()) {
      ...
      if (fileSection(root.addElement("fileSec"))) {
        ...
        setRels(root.addElement("structMap"));
        if (memberType.equals("videos")) {
  ...
  /**
   * 
   * Part - Display, MASTER, MASTER-RETOUCHED Label Type Position image/video
   * 
   * @return
   */
  public List<List<String>> getParts() {
    ...
    for (HashMap<String, String> thisPart : hasVideos) {
    ...
    for (String thisPart : hasRelations) {
  ...
  private void parseDoc(Document METSsource) {
    ...
    for (Object node : lNodes) {
      ...
      eSubNode = (Element) DoUtils.getXPath("mets:FLocat").selectSingleNode(
          eNode);
      strPid = eSubNode.attributeValue("href");

      if (strPid == null) {
        strPid = eSubNode.attributeValue("xlink:href");
      }
    ...
    lNodes = DoUtils.getXPath("/mets:mets/mets:structMap/mets:div")
        .selectNodes(METSsource);
    ...
    for (Object node : lNodes) {
      ...
      /*
       * Each part can consist of up to 3 types (IDs) DISPLAY, MASTER, and
       * MASTER_RETOUCHED.
       * 
       * Image/Book METS nodes look like this <div LABEL="front" ORDER="1"> <div
       * ID="DISPLAY"> <fptr FILEID="2"/> </div> <div ID="MASTER-RETOUCHED">
       * <fptr FILEID="3"/> </div> </div>
       */
      ...
      for (Object subNode : lSubNodes) {
        ...
        eSubNode = (Element) DoUtils.getXPath("mets:fptr").selectSingleNode(
            eSubNode);
    ...
    for (Object node : lNodes) {
       ...
       * 
       * Video METS nodes look like this <div ORDER="1"> <fptr FILEID="2"/>
       * </div>
       */
      eSubNode = (Element) DoUtils.getXPath("mets:fptr")
    ...
    /*
     * So we have now populated collections hasImages hasVideos hasRelations Now
     * we can return you to the regularly scheduled program.
     */
  }
  ...
  private void setFLocat(Element fileGrp, String thisOne, int fileCount) {
    ...
    Element eTemp;
    // ArrayList<HashMap<String, String>> hasParts = new
    ...
    thisQ = new QName("ID", nsMets);
    file.addAttribute(thisQ, String.valueOf(fileCount));

    thisQ = new QName("FLocat", nsMets);
    eTemp = file.addElement(thisQ);

    thisQ = new QName("LOCTYPE", nsMets);
    eTemp.addAttribute(thisQ, "OTHER");
    thisQ = new QName("OTHERLOCTYPE", nsMets);
    eTemp.addAttribute(thisQ, "PID");
    thisQ = new QName("href", nsXlink);
    eTemp.addAttribute(thisQ, thisOne);
    thisQ = new QName("type", nsXlink);
    eTemp.addAttribute(thisQ, "simple");
  ...
  private void setRels(Element relsSec) {
    ...
    div.addAttribute("ID", "isMemberOfCollection");
  ...
  private void setImages(Element partsSec) {
    ...
    if (hasImages.size() > 0) {
      ...
      // Process the members
      Element div = partsSec.addElement(divQ).addAttribute("ID", memberType);
      ...
      for (HashMap<String, String> thisPart : hasImages) {
        ...
        for (String sType : aTypes) {
          if (thisPart.get(sType) != null) {
            ...
            if (sTempStr != null && sTempStr.length() > 0) {
              part.addElement(divQ).addAttribute("ID", sType).addElement(fptrQ)
  ...
  private void setVideos(Element partsSec) {
    ...
    if (hasVideos.size() > 0) {
      ...
      // Process the members
      Element div = partsSec.addElement(divQ).addAttribute("ID", "videos");
  ...
  private void setRelations(Element relationsSec) {
    ...
    if (hasRelations.size() > 0) {
      ...
      Element div = relationsSec.addElement(divQ).addAttribute("ID", "related");
      ...
      for (String strKey : hasRelations) {
        ...
        if (sTempStr != null && sTempStr.length() > 0) {
          ...
          div.addElement(divQ).addAttribute("ORDER", String.valueOf(position))
              .addElement(fptrQ).addAttribute("FILEID", sTempStr);
        }
      }
    }
  }
  ...
  public static void main(String[] args) {
    ...
    DoUtils.saveDoc(thisMETS, "terminal");

    hereTis.addPart(3, "umd:22", "DISPLAY", "Page 3A");

    thisMETS = hereTis.getXML();
